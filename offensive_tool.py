    ...
    try:
        ...
        for arg in sys.argv[1:]:
            if arg.startswith("--config="):
                ...
                pass  # Ignore unknown arguments
            elif "://" in arg:
                # Parse URL format: https://example.com or example.com:443
                target_host = arg
        ...
        if target_host:
            # Parse target_host (could be URL or host:port)
            if "://" in target_host:
                # Strip protocol
            ...
            if ":" in target_host and not target_host.startswith("["):
                ...
                try:
                    ...
                    config_data["host"] = target_host
            else:
                config_data["host"] = target_host
        
        if ignore_ssl:
            config_data["verify_ssl"] = False
            logger.warning("SSL verification disabled via --ignore-ssl flag")
        
        # Ensure required fields have defaults
        config_data.setdefault("host", "target.com")
        config_data.setdefault("port", 443)
        config_data.setdefault("timeout", 30)
        config_data.setdefault("verify_ssl", True)
        config_data.setdefault("rate_limit", 10)
        config_data.setdefault("stealth_mode", True)
        
        # Create config object
        config = OffensiveConfig(**config_data)
        
        logger.info(f"Target: {config.host}:{config.port}")
        logger.info(f"SSL Verification: {config.verify_ssl}")
        logger.info(f"Rate Limit: {config.rate_limit} req/s")
        logger.info(f"Stealth Mode: {config.stealth_mode}")
